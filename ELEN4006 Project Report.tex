...
To ensure the aliased signals and noise below the resolution of the converter, the aliased signals must be attenuated by $74~dB$, which is calculated from \cref{eqn:snr}, to ensure they are below half the system's least significant bit (LSB) \cite{alias-error}. This should be achieved at half the sampling frequency based on Nyquist's theorem. From \cref{fig:bode} it can be seen that this occurs at $21.645~kHz$. This is well below half of the $100~\mathrm{kHz}$ sample rate of the PIC, thus it can be deduced that negligible aliasing error is incurred and there is no need for a separate ADC component in the system.
